...
private func setLabelAccessibleTextColor(_ label: UILabel,
  ...
  if (backgroundColor != nil) {
    let options : GTFTextAccessibilityOptions = [
      GTFTextAccessibility.isLarge(forContrastRatios: label.font) ? .largeFont : GTFTextAccessibilityOptions(),
      preferLightText ? .preferLighter : .preferDarker
    ]

    label.textColor =
      GTFTextAccessibility.textColor(onBackgroundColor: backgroundColor!,
...
private func contrastRatioTitle(_ prefix: String,
  ...
  let ratio = GTFTextAccessibility.contrastRatio(forTextColor: textColor,
